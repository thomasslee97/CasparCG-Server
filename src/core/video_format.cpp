...
struct video_format_registry::impl
{
  private:
...
video_format_registry::video_format_registry()
    : impl_(new impl())
{
}
video_format_desc video_format_registry::invalid()
{
    return video_format_desc(video_format::invalid, 1, 0, 0, 0, 0, 1, 1, L"invalid", {1});
};
video_format_desc video_format_registry::find(const std::wstring& name) const { return impl_->find(name); }
video_format_desc video_format_registry::find_format(const video_format& format) const
{
    return impl_->find_format(format);
}
void video_format_registry::store(const video_format_desc& format) { impl_->store(format); }
...
{
    *this = video_format_registry::invalid();
}
