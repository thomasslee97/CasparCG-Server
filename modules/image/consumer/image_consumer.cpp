...
void write_cropped_png(const core::const_frame&       frame,
                       ...
                       int                            height)
{
    ...
    image_view<bgra_pixel> complete_frame(
        const_cast<uint8_t*>(frame.image_data().begin()), format_desc.width, format_desc.height);
...
void describe_consumer(core::help_sink& sink, const core::help_repository& repo)
{
    ...
    sink.para()
        ->text(L"Writes a single PNG snapshot of a video channel. ")
        ->code(L".png")
        ->text(L" will be appended to ")
        ->code(L"filename")
        ->text(L". The PNG image will be stored under the ")
        ->code(L"media")
        ->text(L" folder.");
